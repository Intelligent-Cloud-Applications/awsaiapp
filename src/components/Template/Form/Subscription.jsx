...
function Subscription({
  ...
  const [subscriptionTypes, setSubscriptionTypes] = useState(
    Array(subscriptions.length).fill('monthly'),
  )

  const [countryCodes, setCountryCodes] = useState(
    Array(subscriptions.length).fill(''),
  )
  useEffect(() => {
    const savedSubscriptionTypes =
      JSON.parse(localStorage.getItem('subscriptionTypes')) ||
      Array(subscriptions.length).fill('monthly')
    const savedCountryCodes =
      JSON.parse(localStorage.getItem('countryCodes')) ||
      Array(subscriptions.length).fill('')
    const savedProvides =
      JSON.parse(localStorage.getItem('provides')) ||
      Array(subscriptions.length).fill([])
  ...
  const removeSubscription = async (indexToRemove) => {
    ...
    if (subscription.productId) {
      try {
        ...
        await API.del(
          'clients',
          `/user/development-form/delete-subscription/${subscription.institution}`,
          {
            body: {
  ...
  return (
    ...
    >
      ...
      <h5 className="w-[28rem] max950:w-[15rem] text-[#cc3f3f] text-[13px]">
        ** The subscription model shown is just an example of how your given
        data will look like for the subscription; it will not change on giving
        your input.**
      </h5>
      <h5 className="w-[28rem] max950:w-[17rem] text-[#939393]">
        Outline subscription plans clearly, emphasizing features and benefits to
        aid user decision-making.
      </h5>
      <div className="mt-4">
        {subscriptions.map((subscription, index) => (
          <div key={index} className="mt-2">
            ...
            <div className="relative">
              ...
              />
              <div className="absolute left-0 right-0  h-[1px] bg-[#939393]"></div>
            </div>
            <div className="relative">
              <select
            ...
            <div className="relative mt-2">
              ...
              <div className="flex items-center">
                ...
                >
                  {subscription.india && (
                    <div className="w-full h-full bg-[#30AFBC] rounded-full" />
                  )}
    ...
    /> */}
              ...
              <div className="mb-10 flex justify-center ">
                <button
                  ...
                  className="bg-[#30AFBC] text-white px-4 py-2 mt-4 rounded-md"
                >
                  Add Service
                </button>
              </div>
