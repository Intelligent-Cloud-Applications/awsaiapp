...
function CashoutTab({ institution }) {
  ...
  return (
    <div className="p-6 relative bg-white shadow-md">
      {/* Currency Financial Overview Row */}
      <div className="grid grid-cols-3 gap-4 mb-4 items-center">
        {Object.entries(currencyBreakdown).map(([currency, data]) => {
          ...
          return (
            <div
              key={currency}
              className=" bg-white border border-gray-200 rounded-xl shadow-md p-5 transform transition-all hover:scale-104 hover:shadow-xl"
            >
        ...
        {/* button */}
        <div
          className="absolute cursor-pointer -top-8 right-4 z-50 bg-green-100 flex px-2 items-center rounded-lg justify-center text-green-700"
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
          ...
          <p className="text-green-700 text-[1rem]">Add Payment</p>
        </div>

        {/* Contact Details */}
        <div className="bg-white border border-gray-200 rounded-xl shadow-md p-5 transform transition-all hover:scale-104 hover:shadow-xl">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center">
              <div className="bg-blue-100 p-2 rounded-full mr-3">
                <svg
          ...
          <div className="grid grid-cols-2 gap-3 text-sm text-gray-600 bg-gray-50 p-3 rounded-lg space-y-2">
            <div className="flex items-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
              ...
              <span>{clientData.contactDetails.phone}</span>
            </div>
            <div className="flex items-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
              ...
              <span>{clientData.contactDetails.email}</span>
            </div>
            <div className="flex items-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
      ...
      <div className="shadow-md border border-gray-200 rounded-md overflow-hidden">
          ...
          <table className="min-w-full divide-y divide-gray-300">
              ...
              <tr>
                {["Transaction ID", "Amount", "Date", "Status"].map(
                  (header) => (
                    <th
                      key={header}
                      className="px-4 py-2 text-left text-sm font-medium text-gray-900 uppercase tracking-wider"
                    >
                      {header}
                    </th>
                  )
            ...
            <tbody>
              {currentItems.map((log, index) => (
                <tr
                  key={index}
                  className={`border-b border-[#ebebeb] hover:bg-gray-${index % 2 === 0 ? "50" : "50"}`}
                >
        ...
        {/* Pagination Controls */}
        {totalPages > 1 && (
          <div className="flex justify-end items-center p-4 space-x-2 text-sm">
            {currentPage > 1 && (
              <button
                onClick={() => handlePageChange(currentPage - 1)}
                className="px-3 py-1 rounded-lg text-black hover:bg-blue-200 transition-colors duration-200 flex items-center"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-5 w-5"
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg transition-all duration-200 ${
                  currentPage === index + 1
                    ...
                    : "bg-gray-50 text-gray-800 hover:bg-blue-200 hover:scale-105"
                }`}
              >
                {index + 1}
              </button>
