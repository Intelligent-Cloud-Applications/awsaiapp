        ...
        <div className="w-[76vw] min-h-[45vh] relative overflow-y-auto max-h-[48vh] scroll-container ml-[rem] pl-[7px] max1050:w-[90vw]">
          {clientsToDisplay.map(([key, pending], index) => (
            <div
              ...
              style={{
                margin: isRowSelected(pending.institution) ? "1rem 0" : "0.5rem 0",
                ...
                <div className=" flex gap-[1rem] pl-[2rem] items-center">
                  ...
                  <div className="grid ml-[3rem] grid-cols-12 items-center w-[55vw]">
                    <div className="col-span-3 flex flex-col max600:w-[10rem]">
                      ...
                      <div className="overflow-auto text-[0.8rem] font-[600] email-hover cursor-pointer">{pending.userType}</div>
                      <div className="overflow-auto text-[0.8rem] font-[600]">{pending.phoneNumber}</div>
                    </div>
                    <div className="col-span-3 ml-[3rem] font-semibold text-sm max600:hidden">{pending.country}</div>
                    ...
                    <div className="col-span-3 ml-[1rem] font-semibold text-sm max850:ml-[1rem] max600:hidden">
                      {pending.country === 'USA' ? `$${pending.recentMonthIncome}` : `₹${pending.recentMonthIncome}`}
                    </div>
                    <div className="flex flex-row justify-between w-[17vw]">
                      <div className="ml-[-1rem] relative font-semibold text-sm max850:ml-[1rem] max600:hidden">{pending.recentMonthMembers}</div>
                      <div className="w-[10rem] ml-[4rem] text-center font-semibold text-sm max600:hidden">{pending.recentMonthAttendance}</div>
                      <div className="w-[10rem] font-semibold text-center text-sm max1300:hidden">{pending.recentMonthLeads}</div>
                    </div>
                  </div>
                </div>
              </div>
