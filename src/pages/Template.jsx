...
const Template = () => {
  ...
  // const [logo, setLogo] = useState(null);
  const [src_Components_Home_Why__h1, setsrc_Components_Home_Why__h1] =
    useState(null)
  const [src_Components_Home_Header3__h1, setsrc_Components_Home_Header3__h1] =
    useState(null)
  const [src_Components_Home_Header3__h2, setsrc_Components_Home_Header3__h2] =
    useState(null)
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (templateResponse) {
          ...
          // SERVICES
          setsrc_Components_Home_Header3__h1(
            templateResponse.src_Components_Home_Header3__h1 || '',
          )
          setsrc_Components_Home_Header3__h2(
            templateResponse.src_Components_Home_Header3__h2 || '',
          )
          setsrc_Components_Home_Why__h1(
            templateResponse.src_Components_Home_Why__h1 || '',
          )

          setServices([
            {
              title: templateResponse.src_Components_Home_Header3__h5_1 || '',
              description:
                templateResponse.src_Components_Home_Header3__p_1 || '',
            },
            {
              title: templateResponse.src_Components_Home_Header3__h5_2 || '',
              description:
                templateResponse.src_Components_Home_Header3__p_2 || '',
            },
            {
              title: templateResponse.src_Components_Home_Header3__h5_3 || '',
              description:
                templateResponse.src_Components_Home_Header3__p_3 || '',
            },
          ])
          ...
          for (let i = 0; i < 3; i++) {
            ...
            test[i].uploadedFile =
              templateResponse.Testimonial[i].uploadedFile.name
  ...
  const handleCompanyUpload = async () => {
    try {
      ...
      const additionalAttributes = {
        LightPrimaryColor:
          LightPrimaryColor !== undefined ? LightPrimaryColor : null,
        LightestPrimaryColor:
          LightestPrimaryColor !== undefined ? LightestPrimaryColor : null,
      }
  ...
  const handleServicesUpload = async () => {
    try {
      const filledDanceTypes = danceTypes
        .filter((type) => type.trim() !== '')
        .slice(0, 5)

      // Pad the array with empty strings to ensure it has a length of 5
      const paddedDanceTypes = filledDanceTypes.concat(
        Array(5 - filledDanceTypes.length).fill(''),
      )

      // Filter out empty strings from the paddedDanceTypes array
      const nonEmptyDanceTypes = paddedDanceTypes.filter(
        (type) => type.trim() !== '',
      )
  ...
  const handleTestimonialsUpload = async () => {
    ...
    try {
      const response1 = await Storage.put(
        `institution-utils/${companyName}/images/Testimonial/${testimonials[0].uploadedFile}`,
        testimonials[0].actualFile,
        {
      ...
      const response2 = await Storage.put(
        `institution-utils/${companyName}/images/Testimonial/${testimonials[1].uploadedFile}`,
        testimonials[1].actualFile,
        {
      ...
      const response3 = await Storage.put(
        `institution-utils/${companyName}/images/Testimonial/${testimonials[2].uploadedFile}`,
        testimonials[2].actualFile,
        {
    ...
    try {
      ...
      for (let i = 0; i < subscriptions.length; i++) {
        ...
        if (subscription.productId) {
          await API.put(
            'clients',
            '/user/development-form/update-subscription',
            {
              body: subscription,
            },
          )
          ...
          const response = await API.put(
            'clients',
            '/user/development-form/subscriptions',
            {
              body: {
    ...
    try {
      ...
      for (let i = 0; i < instructors.length; i++) {
        ...
        if (instructor.actualFile) {
          const response = await Storage.put(
            `institution-utils/${companyName}/images/Instructor/${instructor.uploadedFile}`,
            instructor.actualFile,
            {
      ...
      for (let i = 0; i < instructors.length; i++) {
        ...
        if (
          ...
          instructor.emailId &&
          instructor.position &&
          (instructor.imgSrc || uploadedImages[i])
        ) {
          try {
            if (instructor.instructorId) {
              await API.put(
                'clients',
                `/user/development-form/update-instructor`,
                {
                  body: {
              ...
              const response = await API.put(
                'clients',
                '/user/development-form/instructor',
                {
                  body: {
                    institution: companyName,
                    name: instructor.name,
    ...
    setCurrentSection((prevSection) => {
      ...
      switch (currentSection) {
        ...
        case 2:
          ...
          break
        case 3:
          const isTestimonialsFilled =
            testimonials.filter(
              (testimonial) => testimonial.name && testimonial.feedback,
            ).length >= 3
          ...
          if (
            ...
            alert(
              'Please fill up all fields for testimonial 3 before proceeding.1',
            )
            return prevSection
          }
          if (
            ...
            alert(
              'Please fill up all fields for testimonial 3 before proceeding.2',
            )
            return prevSection
          }
          if (
            ...
            alert(
              'Please fill up all fields for testimonial 3 before proceeding.',
            )
            return prevSection
          }
          ...
          break
        case 4:
          const invalidPriceIndex = subscriptions.findIndex((subscription) =>
            isNaN(Number(subscription.amount)),
          )
          if (invalidPriceIndex !== -1) {
            alert(
              `Please enter a valid price number for subscription ${invalidPriceIndex + 1}.`,
            )
            return prevSection
          }
        ...
        case 5:
          ...
          if (!allFAQsFilled) {
            alert(
              'Please fill both the question and answer for each FAQ before proceeding.',
            )
            return prevSection
          }
          handleFAQsUpload()
        ...
        case 6:
          ...
          if (incompleteIndex !== -1) {
            ...
            if (
              !incompleteInstructor.name ||
              !incompleteInstructor.emailId ||
              !incompleteInstructor.position ||
              !incompleteInstructor.actualFile
            ) {
              alert(
                `Please fill all fields for instructor ${incompleteIndex + 1} before proceeding.`,
              )
              return prevSection
            }
          }
    ...
    >
      ...
      <div className="flex-grow flex">
        ...
        <div className=" w-4/7 pt-[6rem] max950:mb-10 max950:w-screen max950:px-14 max600:px-0 right-20 fixed respo">
          ...
          {currentSection === 5 && <FAQs faqs={faqs} setFaqs={setFaqs} />}
          ...
          {currentSection === 7 && (
            <Policy policies={policies} setPolicies={setPolicies} />
          )}
        ...
        <div
          style={{ position: 'fixed', width: '100%', bottom: 0, zIndex: 99 }}
        >
          <Footer
            saveData={saveData}
            currentSection={currentSection}
