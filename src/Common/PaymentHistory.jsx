...
const PaymentHistory = ({ institution }) => {
  ...
  return (
    <div className="w-full h-screen flex justify-center items-center">
      <div className="bg-white shadow-md rounded-lg overflow-x-auto overflow-y-auto w-[80vw] min-h-[80vh] max600:h-screen max600:mt-[8rem]">
        <div className="hidden md:grid grid-cols-7 gap-4 p-4 text-white" style={{ backgroundColor: color.primary }}>
        ...
        {payments.length > 0 ? (
          payments.map((payment, index) => (
            <div key={index} className="border-t border-b p-3 md:border-none border-black grid grid-cols-1 md:grid-cols-7 py-6 items-center max600:border max600:border-black max600:mt-[1rem]">
              ...
              <div className="col-span-1 flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-4">
                <div className="font-bold md:hidden">Payment Date:</div>
                <div>{formatDate(payment.paymentDate)}</div>
              </div>
              <div className="col-span-1 flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-4">
                <div className="font-bold md:hidden">Renew Date:</div>
                <div>{formatDate(payment.renewDate)}</div>
              </div>
              <div className="col-span-1 flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-4">
                <div className="font-bold md:hidden">Amount:</div>
