...
useEffect(() => {
  if (isAuth) {
    const onLoad = async () => {
      try {
        const classes = await API.get("user", "/user/upcoming-schedule/awsaiapp");
      ...
      try {
        const classes = await API.get("user", "/user/previous-schedule/awsaiapp");
      ...
      try {
        const list = await API.get("user", "/admin/profile-list/awsaiapp");
        setUserList(list);
      } catch (e) {
        console.log(e);
        setUserList([]);
